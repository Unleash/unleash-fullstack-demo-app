...
export const userExpenses: Expense[] = [
  { id: 1, category: 'Food', amount: 125.5, date: '2023-05-01' },
  { id: 2, category: 'Transportation', amount: 45.0, date: '2023-05-02' },
  { id: 3, category: 'Entertainment', amount: 65.75, date: '2023-05-03' },
  { id: 4, category: 'Utilities', amount: 120.0, date: '2023-05-05' },
  { id: 5, category: 'Shopping', amount: 250.3, date: '2023-05-07' }
]

export function generateBasicChatResponse(
  message: string,
  expenses: Expense[]
): string {
  ...
  if (lowerMessage.includes('total expenses')) {
    ...
    return `Your highest expense is $${highest.amount.toFixed(2)} for ${
      highest.category
    } on ${highest.date}.`
  } else {
...
export function generateAdvancedChatResponse(
  message: string,
  expenses: Expense[]
): string {
  const lowerMessage = message.toLowerCase()
  ...
  if (lowerMessage.includes('total expenses')) {
    ...
    return `Your total expenses are $${total.toFixed(
      2
    )}, with an average of $${average.toFixed(2)} per transaction.`
  } else if (lowerMessage.includes('categories')) {
    ...
    const percentOfTotal =
      (highest.amount / expenses.reduce((sum, exp) => sum + exp.amount, 0)) *
      100
    return `Your highest expense is $${highest.amount.toFixed(2)} for ${
      highest.category
    } on ${highest.date}. This represents ${percentOfTotal.toFixed(
      1
    )}% of your total expenses.`
  } else if (
    lowerMessage.includes('spending pattern') ||
    ...
    response += `- Your spending started at $${sortedExpenses[0].amount.toFixed(
      2
    )} on ${sortedExpenses[0].date}\n`
    response += `- Your most recent expense was $${sortedExpenses[
      sortedExpenses.length - 1
    ].amount.toFixed(2)} on ${sortedExpenses[sortedExpenses.length - 1].date}\n`

    const firstHalfAvg =
      sortedExpenses
        .slice(0, Math.floor(sortedExpenses.length / 2))
        .reduce((sum, exp) => sum + exp.amount, 0) /
      Math.floor(sortedExpenses.length / 2)
    const secondHalfAvg =
      sortedExpenses
        .slice(Math.floor(sortedExpenses.length / 2))
        .reduce((sum, exp) => sum + exp.amount, 0) /
      (sortedExpenses.length - Math.floor(sortedExpenses.length / 2))

    if (secondHalfAvg > firstHalfAvg) {
      response += `- Your spending is trending upward by ${(
        (secondHalfAvg / firstHalfAvg - 1) *
        100
      ).toFixed(1)}%`
    } else {
      response += `- Your spending is trending downward by ${(
        (1 - secondHalfAvg / firstHalfAvg) *
        100
      ).toFixed(1)}%`
    }
